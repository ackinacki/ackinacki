...
fn main() {
    ...
    } else if cfg!(all(feature = "sqlcipher", not(feature = "bundled-sqlcipher"))) {
...
#[cfg(any(feature = "bundled", feature = "bundled-windows", feature = "bundled-sqlcipher"))]
mod build_bundled {
    use std::env;
    ...
    fn from(header: HeaderLocation) -> Self {
        match header {
            ...
            HeaderLocation::Wrapper => {
                if cfg!(feature = "loadable_extension") { "wrapper_ext.h" } else { "wrapper.h" }
                    .into()
            }
            HeaderLocation::FromPath(path) => format!(
                "{}/{}",
                path,
                if cfg!(feature = "loadable_extension") { "sqlite3ext.h" } else { "sqlite3.h" }
            ),
        }
    }
}
...
mod build_linked {
    ...
    fn find_sqlite() -> HeaderLocation {
        ...
        if let Ok(dir) = env::var(format!("{}_LIB_DIR", env_prefix())) {
            ...
            #[cfg(not(feature = "loadable_extension"))]
            if pkg_config::Config::new().atleast_version("3.14.0").probe(link_lib).is_err() {
...
mod bindings {
    ...
    pub fn sqlite3_cancel_auto_extension(
        ...
        #[cfg(feature = "loadable_extension")]
        {
            ...
            bindings.write(Box::new(&mut output)).expect("could not write output of bindgen");
...
mod loadable_extension {
    ...
    pub fn generate_functions(output: &mut String) {
        ...
        for field in sqlite3_api_routines.fields {
            ...
            output.push('\n');
            if name == "malloc" { &mut malloc } else { &mut stores }.push(quote::quote! {
