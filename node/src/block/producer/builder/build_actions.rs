...
impl BlockBuilder {
    ...
    pub(super) fn after_transaction(&mut self, thread_result: ThreadResult) -> anyhow::Result<()> {
        ...
        let is_tx_aborted = transaction
            ...
            .is_aborted();
        if is_tx_aborted && thread_result.in_msg_is_ext {
            tracing::trace!("Ext message was aborted, do not process resulting tx");
            return Ok(());
        }
    ...
    pub(super) fn execute(
        ...
        let thread =
            std::thread::Builder::new().name("Message execution".to_string()).spawn(move || {
                ...
                res.map(|(tx, lt, trace, minted_shell, is_ext_message)| ThreadResult {
                    transaction: tx,
                    lt,
                    trace,
    ...
    pub fn build_block(
        ...
        trace_span!("internal messages execution", messages.len = sorted.len() as i64)
            .in_scope(|| {
...
Ok::<_, anyhow::Error>(())
    ...
    fn finish_block(
        ...
        let new_state = OptimisticStateImpl::builder()
            ...
            .threads_table(self.initial_optimistic_state.get_produced_threads_table().clone())
    ...
    fn add_new_message(&mut self, mut message: Message, tr_cell: Option<Cell>) {
        ...
        let body_opt = message.clone().body();
        if let Some(data) = message.int_header_mut() {
