...
#[derive(Debug, Clone, Eq, PartialEq, Getters, Serialize, Deserialize, Hash)]
...
impl From<&Envelope<AckiNackiBlock>> for BlockIndex {
    fn from(acki_block: &Envelope<AckiNackiBlock>) -> Self {
        Self {
            block_seq_no: acki_block.data().seq_no(),
            block_identifier: acki_block.data().identifier(),
        }
    }
}
