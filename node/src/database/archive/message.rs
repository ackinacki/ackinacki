...
use crate::database::serialize_block::MessageSerializationSet;
...
impl From<MessageSerializationSet> for ArchMessage {
    fn from(msg: MessageSerializationSet) -> Self {
        log::debug!("msg conversion start: {}", msg.id.to_hex_string());
        ...
        if let Some(state) = msg.message.state_init() {
            if let Some(cell) = state.code() {
                if !cell.is_pruned() {
                    let bytes = write_boc(cell).expect("Failed to write boc of the message code");
                    code = Some(bytes);
            ...
            if let Some(cell) = state.data() {
                if !cell.is_pruned() {
                    let bytes = write_boc(cell).expect("Failed to write boc of the message data");
                    data = Some(bytes);
        ...
        let arch_msg = match msg.message.header() {
            CommonMsgInfo::IntMsgInfo(header) => Self {
                ...
                value_other: Some(
                    write_boc(&header.value.other.serialize().expect("Failed to serialize ECC"))
                        .expect("Failed to create BOC"),
                ),
        ...
        log::debug!("msg conversion end: {}", msg.id.to_hex_string());
