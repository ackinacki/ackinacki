...
impl ActionLockStorage {
    ...
    pub fn read_blob<T: for<'de> Deserialize<'de>>(&self, path: &str) -> anyhow::Result<Option<T>> {
        ...
        if let Some(bins) =
            self.store.get(key, &[BIN_BLOB].into(), AEROSPIKE_OBJECT_TYPE_CROSS_ACTION_LOCK)?
        {
    ...
    pub fn write_blob<T: Serialize>(
        ...
        self.store.put(key, bins, until_success, AEROSPIKE_OBJECT_TYPE_CROSS_ACTION_LOCK)?;
        Ok(())
    }
}
