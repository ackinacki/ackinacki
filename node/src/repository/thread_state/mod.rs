...
impl ThreadState {
    ...
    pub fn move_finalized(self, next_finalized_block: Envelope<AckiNackiBlock>) -> Self {
        let ThreadState { last_finalized } = self;
        //
        let next_checkpoint = last_finalized.apply(next_finalized_block).unwrap();
        Self { last_finalized: next_checkpoint }
    }

    pub fn move_prefinalized(
        self,
        next_prefinalized_block: Envelope<AckiNackiBlock>,
    ) -> Self {
    }
}
