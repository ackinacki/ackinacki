...
impl Display for BlockProtocolVersionState {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            BlockProtocolVersionState::Current(protocol_version) => {
                write!(f, "{}", protocol_version)
            }
            BlockProtocolVersionState::TransitionLocked { from, to, milestone: _ } => {
                write!(f, "{}->{}", from, to)
            }
