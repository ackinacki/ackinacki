...
mod tests {
    ...
    fn ensure_repository_returns_same_ref_for_the_same_key() {
        ...
        let value_a = repository.get(some_block_id.clone()).unwrap();
        let value_b = repository.get(some_block_id.clone()).unwrap();
    ...
    fn ensure_repository_cleans_up_expired_entries_on_access() {
        ...
        assert!(repository.map.lock().is_empty());
        let value_a = repository.get(some_block_id.clone()).unwrap();
        let value_b = repository.get(some_other_block_id.clone()).unwrap();
        // ensure table stores values
        assert!(repository.map.lock().len() == 2);
        drop(value_a);
        drop(value_b);
        let _ = repository.get(some_block_id.clone()).unwrap();
