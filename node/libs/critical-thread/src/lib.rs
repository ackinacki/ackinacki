...
impl SpawnCritical for std::thread::Builder {
        ...
        T: Send + 'static,
    {
        self.spawn(move || match f() {
            ...
            Err(e) => {
                ...
                tracing::error!(target: "monit", "Critical <{name}> thread exited with an error: {e}");
                eprintln!("Critical <{name}> thread exited with an error: {e}");
        ...
        T: Send + 'scope,
    {
        self.spawn_scoped(scope, move || match f() {
            ...
            Err(e) => {
                let name = std::thread::current().name().unwrap_or("unknown").to_owned();
                tracing::error!(target: "monit", "Critical scoped <{name}> thread exited with an error: {e}");
                eprintln!("Critical <{name}> thread exited with an error: {e}");
                std::process::exit(101);
            }
        })
